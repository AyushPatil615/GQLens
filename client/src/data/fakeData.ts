// ─── Fake Event Log (Phase 1 hardcoded data) ──────────────────────
// Each step has: timing + caption (subtitle) + a full rich dialogue

export type EventStep = {
  step: string;
  ms: number;
  caption: string; // short one-liner shown in timeline
};
...
export const FAKE_EVENT_LOG: EventStep[] = [
  {
    step: 'parse',
    ms: 5,
    caption: 'Reading your query text and turning it into a structure the computer understands.',
  },
  {
    step: 'validate',
    ms: 4,
    caption: "Checking that 'name', 'age', and 'courses' are real fields that exist in the schema.",
  },
  {
  ...
  },
  {
    ...
    ms: 9,
    caption: 'Running the Courses resolver — fetching which classes this student is enrolled in.',
  },
  {
    step: 'respond',
    ms: 3,
